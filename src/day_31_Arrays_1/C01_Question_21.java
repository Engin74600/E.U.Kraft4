       ...
       int i=0;
        while(true){
            ...
            if (kelime1.length()>0 && kelime1.length()<5){
               break;
            }//ife girip de sonuc varırsa zaten whilw biter, soksa aşağıya iner
...
// iki sayının ortak böleni
public static int ebob(int x,int y){
    int result=1;
    int min=Math.min(x,y);
    for (int i = 1; i <= min ; i++) {//sıfır modu olmaz. o yüzden 1
        if(x%i==0 && y%i==0){
            result=i;
        }
    }
...
//Not: formül a*b= ebob(a,b)*ekok(a,b) ise
